...
```bash
# 1. Place your video
cp ~/Downloads/bug-report.mov ~/video-input-code-skill-storage/

# 2. In Claude Code, run the skill
/video-input-code-skill

# 3. Optionally adjust keyframe detection threshold (0-100, default: 1)
#    Use -d_t or -detection_threshold
/video-input-code-skill -d_t 5

# 4. Optionally run the skill with action points
/video-input-code-skill and find instances when presenter was asking for feedback and describe what was shown in the video at that time
```
...
Record your screen while narrating a bug report, feature request, etc. then save the video file to `~/video-input-code-skill-storage/`. The skill will:
...
cp ~/Downloads/user-flow-walkthrough.mov ~/video-input-code-skill-storage/

# 3. Run the skill with an instruction to generate Playwright code
/video-input-code-skill Based on this recording, create a Playwright E2E test that automates the exact sequence of actions shown and then refactor it to the Page Object Model
```
...
If no new video is in `~/video-input-code-skill-storage/`, the skill automatically loads the most recent archived analysis:
...
When multiple videos exist in `~/video-input-code-skill-storage/`, the skill picks the **latest by modification time**. To analyze a specific video, remove or move the others first.
